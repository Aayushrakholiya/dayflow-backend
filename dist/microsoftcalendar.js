...
exports.default = createMicrosoftCalendarRouter;
...
async function getValidAccessToken(userId) {
    const tokenRow = await db_1.db.externalCalendarToken.findUnique({
    ...
    if (isExpired && tokenRow.refreshToken) {
        ...
        await db_1.db.externalCalendarToken.update({
            where: { userId_provider: { userId, provider: "microsoft" } },
            data: {
...
function createMicrosoftCalendarRouter() {
    ...
    router.get("/callback", async (req, res) => {
        ...
        try {
            ...
            await db_1.db.externalCalendarToken.upsert({
    ...
    router.delete("/disconnect", async (req, res) => {
        ...
        if (!userId)
            return res.status(400).json({ message: "userId required" });
        try {
            await db_1.db.externalCalendarToken.deleteMany({
                where: { userId, provider: "microsoft" },
            });
            await (0, Importedevents_1.deleteImportedEventsBySource)(userId, "microsoft");
...
// ── Import logic ──────────────────────────────────────────────────────────────
async function importMicrosoftEvents(userId, accessToken) {
    const now = new Date();
    ...
    await (0, Importedevents_1.upsertImportedEvents)(userId, eventsToUpsert);
