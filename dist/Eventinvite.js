...
exports.registerEventInviteRoutes = registerEventInviteRoutes;
// ── Helpers ───────────────────────────────────────────────────────────────────
...
// ── Brevo sender ──────────────────────────────────────────────────────────────
async function sendInviteEmail(to, eventTitle, dateIso, startHour, endHour, location, videoconferencing, description) {
...
// ── Route registration ────────────────────────────────────────────────────────
function registerEventInviteRoutes(app) {
    /**
     * POST /api/events/send-invites
     * Body: { eventTitle, date, startHour, endHour, attendees, location?, videoconferencing?, description? }
     */
    app.post("/api/events/send-invites", async (req, res) => {
        const { eventTitle, date, startHour, endHour, attendees, location, videoconferencing, description, } = req.body;
        if (!eventTitle || !date || startHour == null || endHour == null) {
            return res
                .status(400)
                .json({
                message: "Missing required fields: eventTitle, date, startHour, endHour",
            });
        }
        ...
        const results = await Promise.allSettled(attendees.map((email) => sendInviteEmail(email, eventTitle, date, startHour, endHour, location, videoconferencing, description)));
        const sent = results.filter((r) => r.status === "fulfilled").length;
        const failed = results
            .filter((r) => r.status === "rejected")
            .map((r, i) => {
            const reason = r.reason;
            console.error(`Failed to send invite to ${attendees[i]}:`, reason?.message ?? reason);
            return attendees[i];
        });
