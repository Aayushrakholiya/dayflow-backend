...
exports.processTaskNotifications = processTaskNotifications;
...
const sentNotifications = new Set(); // Track sent notifications to prevent duplicates
async function processEventNotifications() {
    try {
        ...
        const allTodayEvents = await db_1.default.event.findMany({
            where: {
                date: {
                    gte: todayStart,
                    lte: todayEnd,
        ...
        const allTodayImportedEvents = await db_1.default.importedEvent.findMany({
            where: {
                date: {
                    gte: todayStart,
                    lte: todayEnd,
        ...
        for (const event of deduplicatedEvents) {
            ...
            const sentCount = await (0, fcmService_1.sendFCMToMultipleDevices)(event.user.deviceTokens, message);
            totalNotifications += sentCount;
            // Mark as sent to prevent duplicate notifications
            sentNotifications.add(notificationKey);
            logging_1.logger.info("Event notification sent", {
                eventId: event.id,
                eventTitle: event.title,
        ...
        logging_1.logger.error("Error processing event notifications", { error });
    }
}
/**
 * Same for tasks
...
async function processTaskNotifications() {
    try {
        ...
        const todayEnd = new Date(now);
        todayEnd.setUTCHours(23, 59, 59, 999);
        const allTodayTasks = await db_1.default.task.findMany({
            where: {
                dueDate: {
                    gte: todayStart,
                    lte: todayEnd,
        ...
        for (const task of upcomingTasks) {
            ...
            const sentCount = await (0, fcmService_1.sendFCMToMultipleDevices)(task.user.deviceTokens, message);
            totalNotifications += sentCount;
            // Mark as sent to prevent duplicate notifications
            sentNotifications.add(notificationKey);
            logging_1.logger.info("Task notification sent", {
                taskId: task.id,
                taskTitle: task.title,
                userId: task.user.id,
        ...
        logging_1.logger.error("Error processing task notifications", { error });
    }
}
/**
